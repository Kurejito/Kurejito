...
namespace Kurejito {
	...
	public interface IPurchaseGateway {
		...
		/// <param name="card">An instance of <see cref="BankCard"/> containing the customer's payment card details.</param>
		/// <returns>A <see cref="PaymentResponse"/> indicating whether the transaction succeeded.</returns>
		PaymentResponse Purchase(string merchantReference, decimal amount, string currency, BankCard card);
	}
}
