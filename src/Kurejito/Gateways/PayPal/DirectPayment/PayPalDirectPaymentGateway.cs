...
namespace Kurejito.Gateways.PayPal.DirectPayment
{
    ...
    public class PayPalDirectPaymentGateway : IPurchaseGateway
    {
        ...
        private readonly IHttpPostTransport _httpTransport;
        ...
        public PaymentResponse Purchase(string merchantReference, decimal amount, string currency, PaymentCard card)
        {
            ...
            var post = _httpTransport.Post(new Uri("https://api-3t.sandbox.paypal.com/nvp"), CreatePurchaseUri(card, amount, currency));
        ...
        private string CreatePurchaseUri(PaymentCard card, decimal amount, string currency)
        {
            ...
            var values = pairs.Select(pair => String.Format("{0}={1}", pair.Key, HttpUtility.UrlEncode(pair.Value)));
